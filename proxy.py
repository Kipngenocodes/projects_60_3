...
HEX_FILTER = ''.join(
    [(len(repr(chr())) ==3) and chr(i) or '.' for i in range(256)]
)
...
def  proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    if len(remote_buffer):
        print("[<==] Received %d bytes from locahost." % len(remote_buffer))
        client_socket.send(remote_buffer)

    while True:
        local_buffer = recieve_from(client_socket)
        if len(local_buffer):
            line = "[==>] Received %d from local_host." % len(local_buffer)
            print(line)
...
def server_loop(local_host, local_port,
    ...
    try:
        ...
        print("[!!] Failed to listento %s :%d " % (local_host, local_port))
        print("[!1] Check for other listening socket or correct permissions")
        sys.exist(0)
